...
TASK = "wolfram_spectral_weight_sensitivity"
ENCODER = "flat"
PRIOR = "spectral"
PRIOR_WEIGHTS = [0.001, 0.005, 0.01, 0.05]
SEEDS = [0, 1, 2, 3, 4]
NUM_EPOCHS = 200
MAX_STEPS = 16
HORIZONS = [1, 2, 4, 8, 16]
...
N_TRAIN = 200
N_EVAL = 50
OUTPUT_JSON = ROOT / "validation_wolfram_spectral_weight_sweep.json"


def make_key(weight: float, seed: int) -> str:
    return f"{ENCODER}|{PRIOR}|w={weight}|seed={seed}"
...
def main() -> None:
    ...
    for weight in PRIOR_WEIGHTS:
        for seed in SEEDS:
            ...
            save_payload(payload)
            print(f"[w={weight}|seed={seed}] H16={rollout_errors[16]:.3f}", flush=True)
