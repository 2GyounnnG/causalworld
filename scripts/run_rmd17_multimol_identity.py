...
DISJOINT_EVAL = False
OUTPUT_PATH = ROOT / "rmd17_multimol_identity.json"
SUMMARY_PATH = ROOT / "rmd17_multimol_identity_summary.json"
...
def make_key(molecule: str, seed: int) -> str:
...
def print_summary(summary: dict[str, Any]) -> None:
    print("\n=== Summary by molecule and prior ===", flush=True)
    print(
        "molecule | prior | n | H=1 | H=8 | H=16 | r_eff | spec_align",
        flush=True,
    )
    for molecule in MOLECULES:
        cell_summary = summary[molecule][PRIOR]
...
def main() -> None:
    ...
    pending_tasks = []
    for molecule, seed in tasks:
        key = make_key(molecule, seed)
        if already_done(all_results.get(key)):
            print(f"[SKIP] {key}", flush=True)
        else:
            pending_tasks.append((molecule, seed))
    ...
    for i, (molecule, seed) in enumerate(pending_tasks, start=1):
        key = make_key(molecule, seed)
