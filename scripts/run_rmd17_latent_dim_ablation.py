...
EXPERIMENT_NAME = "rmd17_latent_dim_ablation"
MOLECULE = "aspirin"
ENCODER = "flat"
...
HORIZONS = [1, 2, 4, 8, 16]
PRIOR_WEIGHT = 0.1
OUTPUT_PATH = ROOT / "rmd17_latent_dim_ablation.json"
CHECKPOINT_DIR = "checkpoints/rmd17_latent_dim_ablation"
...
def make_key(prior: str, latent_dim: int, seed: int) -> str:
    return f"{MOLECULE}|{ENCODER}|{prior}|dim={latent_dim}|seed={seed}"
...
def main() -> None:
    ...
    for prior, graph_source in PRIOR_VARIANTS:
        for latent_dim in LATENT_DIMS:
            for seed in SEEDS:
                key = make_key(prior, latent_dim, seed)
                ...
                config = Config(
                    ...
                    disjoint_eval=True,
                    checkpoint_dir=f"{CHECKPOINT_DIR}/dim{latent_dim}",
                )
