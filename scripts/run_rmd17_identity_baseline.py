...
OUTPUT_PATH = ROOT / "rmd17_identity_baseline.json"
...
def build_summary(all_results: dict[str, Any]) -> dict[str, Any]:
    ...
    for horizon in [1, 8, 16]:
        values = [get_metric(result.get("rollout_errors", {}), horizon) for result in successful]
        mean, std = mean_std(values)
        summary["rollout_errors"][str(horizon)] = {"mean": mean, "std": std}
    ...
    for diagnostic in [
        ...
        values = [get_metric(result.get("final_diagnostics", {}), diagnostic) for result in successful]
        mean, std = mean_std(values)
        summary["diagnostics"][diagnostic] = {"mean": mean, "std": std}

    return summary


def print_summary(summary: dict[str, Any]) -> None:
    rollouts = summary["rollout_errors"]
    diagnostics = summary["diagnostics"]
    print("\n=== Summary ===", flush=True)
    print(
        f"{'prior':20s} {'n':>3s} {'H=1 mean+-std':>18s} "
        f"{'H=8 mean+-std':>18s} {'H=16 mean+-std':>18s} "
        f"{'r_eff':>8s} {'cond':>10s} {'proj_g':>10s} {'spec_a':>10s}",
        flush=True,
    )
    print(
        f"{PRIOR:20s} {summary['n_success']:3d} "
        ...
        f"{format_float(diagnostics['projection_gaussianity']['mean'], 2):>10s} "
        f"{format_float(diagnostics['spectral_alignment']['mean'], 2):>10s}",
        flush=True,
    )
...
def main() -> None:
    ...
    print(f"Resume: {len(pending_seeds)}/{len(SEEDS)} runs pending", flush=True)
    print(f"output: {OUTPUT_PATH}", flush=True)

    for i, seed in enumerate(pending_seeds, start=1):
        key = make_key(seed)
        print(f"\n=== Starting {key} ({i}/{len(pending_seeds)}) ===", flush=True)
        t0 = time.time()
        config = Config(
            molecule=MOLECULE,
            encoder=ENCODER,
