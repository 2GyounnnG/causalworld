...
from scripts.run_lj_mechanism import (
    DATASETS as _LJ_DATASETS,
    DISJOINT_EVAL,
    ENCODER,
    GRAPH_SOURCE,
    LAPLACIAN_MODE,
    ...
    EVAL_N_TRANSITIONS,
    already_done,
    build_summary as _unused_build_summary,
    config_prior,
    format_float,
    format_mean_std,
    get_metric,
...
DATASETS = ["ho_random", "ho_lattice", "ho_scalefree"]
OUTPUT_PATH = ROOT / "ho_mechanism.json"
SUMMARY_PATH = ROOT / "ho_mechanism_summary.json"

_ = _LJ_DATASETS


def make_key(dataset: str, prior: str, seed: int) -> str:
    return f"{dataset}|{ENCODER}|{prior}|seed={seed}"
...
def build_summary(all_results: dict[str, Any]) -> dict[str, Any]:
    ...
    for dataset in DATASETS:
        ...
        for prior in PRIORS:
            cell_results = [
                all_results.get(make_key(dataset, prior, seed))
                for seed in SEEDS
                if isinstance(all_results.get(make_key(dataset, prior, seed)), dict)
                and "error" not in all_results.get(make_key(dataset, prior, seed), {})
                and already_done(all_results.get(make_key(dataset, prior, seed)))
            ]
            rollout_summary = {
...
def main() -> None:
    ...
    tasks = [(dataset, prior, seed) for dataset in DATASETS for prior in PRIORS for seed in SEEDS]
    all_results = load_results()
    pending_tasks = []
    for dataset, prior, seed in tasks:
        key = make_key(dataset, prior, seed)
        if already_done(all_results.get(key)):
            print(f"[SKIP] {key}", flush=True)
        else:
            pending_tasks.append((dataset, prior, seed))
    ...
    for i, (dataset, prior, seed) in enumerate(pending_tasks, start=1):
        key = make_key(dataset, prior, seed)
