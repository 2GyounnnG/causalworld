...
EXPERIMENT_NAME = "rmd17_hidden_dim_ablation"
MOLECULE = "aspirin"
ENCODER = "flat"
...
HORIZONS = [1, 2, 4, 8, 16]
PRIOR_WEIGHT = 0.1
OUTPUT_PATH = ROOT / "rmd17_hidden_dim_ablation.json"
CHECKPOINT_DIR = "checkpoints/rmd17_hidden_dim_ablation"
...
def make_key(prior: str, hidden_dim: int, seed: int) -> str:
    return f"{MOLECULE}|{ENCODER}|{prior}|hidden={hidden_dim}|seed={seed}"
...
def main() -> None:
    ...
    for prior, graph_source in PRIOR_VARIANTS:
        for hidden_dim in HIDDEN_DIMS:
            for seed in SEEDS:
                key = make_key(prior, hidden_dim, seed)
                ...
                config = Config(
                    ...
                    disjoint_eval=True,
                    checkpoint_dir=f"{CHECKPOINT_DIR}/hidden{hidden_dim}",
                )
