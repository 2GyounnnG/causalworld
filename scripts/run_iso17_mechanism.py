...
from scripts.iso17_loader import ISO17Trajectory, collect_iso17_transitions
from scripts.train_rmd17 import Config, train_one_seed


ISO17_DATASET = "iso17_default"
PRIORS = [
    "none",
    "variance",
    "covariance",
...
DISJOINT_EVAL = False
OUTPUT_PATH = ROOT / "iso17_mechanism.json"
SUMMARY_PATH = ROOT / "iso17_mechanism_summary.json"

_ = ISO17Trajectory, collect_iso17_transitions
...
def make_key(prior: str, seed: int) -> str:
    return f"iso17|{ENCODER}|{prior}|seed={seed}"
...
def format_mean_std(stats: dict[str, float]) -> str:
    return f"{format_float(stats['mean'], 4)}+-{format_float(stats['std'], 4)}"


def build_summary(all_results: dict[str, Any]) -> dict[str, Any]:
    summary: dict[str, Any] = {ISO17_DATASET: {}}
    horizons = [1, 2, 4, 8, 16]
    ...
    for prior in PRIORS:
        cell_results = []
        for seed in SEEDS:
            result = all_results.get(make_key(prior, seed))
            if isinstance(result, dict) and "error" not in result and already_done(result):
                cell_results.append(result)

        rollout_summary = {}
        for horizon in horizons:
            values = [get_metric(result.get("rollout_errors", {}), horizon) for result in cell_results]
            rollout_summary[str(horizon)] = nan_stats(values)

        diagnostic_summary = {}
        for diagnostic in diagnostics:
            values = [get_metric(result.get("final_diagnostics", {}), diagnostic) for result in cell_results]
            stats = nan_stats(values)
            diagnostic_summary[diagnostic] = {"mean": stats["mean"], "std": stats["std"]}

        summary[ISO17_DATASET][prior] = {
...
def print_summary(summary: dict[str, Any]) -> None:
    ...
    for prior in PRIORS:
        cell_summary = summary[ISO17_DATASET][prior]
        rollouts = cell_summary["rollout_errors"]
        diagnostics = cell_summary["diagnostics"]
        print(
            f"{ISO17_DATASET} | {prior} | {cell_summary['n_success']} | "
...
def main() -> None:
    ...
    for i, (prior, seed) in enumerate(pending_tasks, start=1):
        ...
        print(f"  -> saved partial; avg={avg:.0f}s/run; ETA={(avg * remaining) / 60:.0f}min", flush=True)
