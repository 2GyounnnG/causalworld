from __future__ import annotations

import json
import math
import os
from pathlib import Path
import sys
import time
import traceback
from typing import Any

import numpy as np
import torch

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from scripts.train_rmd17 import Config, train_one_seed


MOLECULES = ["ethanol", "malonaldehyde", "naphthalene", "toluene"]
PRIORS = ["permuted_spectral", "random_spectral"]
WEIGHTS = [0.001, 0.01, 0.1]
SEEDS = list(range(3))
NUM_EPOCHS = 50
N_TRANSITIONS = 2000
ENCODER = "flat"
GRAPH_SOURCE = "bond"
LAPLACIAN_MODE = "per_frame"
SAVE_CHECKPOINT = True
DISJOINT_EVAL = False
OUTPUT_PATH = ROOT / "rmd17_multimol_control_weight.json"
SUMMARY_PATH = ROOT / "rmd17_multimol_control_weight_summary.json"


def config_prior(prior: str) -> str:
    return "euclidean" if prior == "covariance" else prior


def make_key(molecule: str, prior: str, weight: float, seed: int) -> str:
    return f"{molecule}|{ENCODER}|{prior}|w={weight}|seed={seed}"


def save_atomic(all_results: dict[str, Any], path: Path) -> None:
    tmp = path.with_suffix(".json.tmp")
    with tmp.open("w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2, default=str)
    os.replace(tmp, path)


def load_results() -> dict[str, Any]:
    if OUTPUT_PATH.exists():
        with OUTPUT_PATH.open("r", encoding="utf-8") as f:
            return json.load(f)
    return {}


def finite_rollout_value(value: Any) -> bool:
    try:
        return math.isfinite(float(value))
    except (TypeError, ValueError):
        return False


def already_done(result: Any) -> bool:
    if not isinstance(result, dict):
        return False
    if "rollout_errors" not in result:
        return False
    rollout_errors = result["rollout_errors"]
    if not isinstance(rollout_errors, dict):
        return False
    return any(finite_rollout_value(value) for value in rollout_errors.values())


def get_metric(mapping: dict[str, Any], key: int | str, default: float = float("nan")) -> float:
    value = mapping.get(key, mapping.get(str(key), default))
    try:
        return float(value)
    except (TypeError, ValueError):
        return float("nan")


def nan_stats(values: list[float]) -> dict[str, float]:
    array = np.asarray(values, dtype=float)
    array = array[np.isfinite(array)]
    n = int(array.size)
    if n == 0:
        return {
            "mean": float("nan"),
            "std": float("nan"),
            "ci_low": float("nan"),
            "ci_high": float("nan"),
        }
    mean = float(np.mean(array))
    std = float(np.std(array, ddof=1)) if n > 1 else 0.0
    ci = 1.96 * std / math.sqrt(n)
    return {
        "mean": mean,
        "std": std,
        "ci_low": mean - ci,
        "ci_high": mean + ci,
    }


def format_float(value: Any, digits: int) -> str:
    try:
        numeric = float(value)
    except (TypeError, ValueError):
        return "nan"
    if not math.isfinite(numeric):
        return "nan"
    return f"{numeric:.{digits}f}"


def format_mean_std(stats: dict[str, float]) -> str:
    return f"{format_float(stats['mean'], 4)}+-{format_float(stats['std'], 4)}"


def build_summary(all_results: dict[str, Any]) -> dict[str, Any]:
    summary: dict[str, Any] = {}
    horizons = [1, 2, 4, 8, 16]
    diagnostics = [
        "effective_rank",
        "condition_number",
        "projection_gaussianity",
        "spectral_alignment",
    ]

    for molecule in MOLECULES:
        summary[molecule] = {}
        for prior in PRIORS:
            summary[molecule][prior] = {}
            for weight in WEIGHTS:
                cell_results = []
                for seed in SEEDS:
                    key = make_key(molecule, prior, weight, seed)
                    result = all_results.get(key)
                    if not isinstance(result, dict) or "error" in result:
                        continue
                    if already_done(result):
                        cell_results.append(result)

                rollout_summary = {}
                for horizon in horizons:
                    values = [
                        get_metric(result.get("rollout_errors", {}), horizon)
                        for result in cell_results
                    ]
                    rollout_summary[str(horizon)] = nan_stats(values)

                diagnostic_summary = {}
                for diagnostic in diagnostics:
                    values = [
                        get_metric(result.get("final_diagnostics", {}), diagnostic)
                        for result in cell_results
                    ]
                    stats = nan_stats(values)
                    diagnostic_summary[diagnostic] = {
                        "mean": stats["mean"],
                        "std": stats["std"],
                    }

                summary[molecule][prior][str(weight)] = {
                    "n_success": len(cell_results),
                    "rollout_errors": rollout_summary,
                    "diagnostics": diagnostic_summary,
                }

    return summary


def print_summary(summary: dict[str, Any]) -> None:
    print("\n=== Summary by molecule, prior, and weight ===", flush=True)
    print(
        "molecule | prior | weight | n | H=1 | H=8 | H=16",
        flush=True,
    )
    for molecule in MOLECULES:
        for prior in PRIORS:
            for weight in WEIGHTS:
                cell_summary = summary[molecule][prior][str(weight)]
                rollouts = cell_summary["rollout_errors"]
                print(
                    f"{molecule} | "
                    f"{prior} | "
                    f"{weight} | "
                    f"{cell_summary['n_success']} | "
                    f"{format_mean_std(rollouts['1'])} | "
                    f"{format_mean_std(rollouts['8'])} | "
                    f"{format_mean_std(rollouts['16'])}",
                    flush=True,
                )


def main() -> None:
    script_start_time = time.time()
    tasks = [
        (molecule, prior, weight, seed)
        for molecule in MOLECULES
        for prior in PRIORS
        for weight in WEIGHTS
        for seed in SEEDS
    ]
    all_results = load_results()

    pending_tasks = []
    for molecule, prior, weight, seed in tasks:
        key = make_key(molecule, prior, weight, seed)
        if already_done(all_results.get(key)):
            print(f"[SKIP] {key}", flush=True)
        else:
            pending_tasks.append((molecule, prior, weight, seed))

    device = "cuda" if torch.cuda.is_available() else "cpu"
    if device == "cuda":
        print(f"device=cuda ({torch.cuda.get_device_name(0)})", flush=True)
    else:
        print("WARNING: CUDA unavailable, falling back to CPU", flush=True)
    print(f"Resume: {len(pending_tasks)}/{len(tasks)} runs pending", flush=True)
    print(f"output: {OUTPUT_PATH}", flush=True)

    for i, (molecule, prior, weight, seed) in enumerate(pending_tasks, start=1):
        key = make_key(molecule, prior, weight, seed)
        print(f"\n=== Starting {key} ({i}/{len(pending_tasks)}) ===", flush=True)
        t0 = time.time()
        config = Config(
            molecule=molecule,
            encoder=ENCODER,
            prior=config_prior(prior),
            graph_source=GRAPH_SOURCE,
            laplacian_mode=LAPLACIAN_MODE,
            seed=seed,
            num_epochs=NUM_EPOCHS,
            n_transitions=N_TRANSITIONS,
            prior_weight=weight,
            save_checkpoint=SAVE_CHECKPOINT,
            disjoint_eval=DISJOINT_EVAL,
        )
        if device == "cpu":
            config.device = "cpu"

        try:
            result = train_one_seed(config)
            if not isinstance(result.get("config"), dict):
                result["config"] = {}
            result["config"]["prior"] = prior
            result["config"]["prior_weight"] = weight
            result["user_facing_prior"] = prior
            result["wall_time_sec"] = time.time() - t0
            all_results[key] = result

            rollout = result.get("rollout_errors", {})
            diag = result.get("final_diagnostics", {})
            print(
                f"=== Finished {key} | "
                f"H1={get_metric(rollout, 1):.4f} "
                f"H8={get_metric(rollout, 8):.4f} "
                f"H16={get_metric(rollout, 16):.4f} | "
                f"r_eff={get_metric(diag, 'effective_rank'):.2f} | "
                f"wall={time.time() - t0:.1f}s ===",
                flush=True,
            )
        except Exception as exc:
            tb = traceback.format_exc()
            all_results[key] = {
                "error": str(exc),
                "traceback": tb,
                "user_facing_prior": prior,
                "wall_time_sec": time.time() - t0,
            }
            print(f"=== FAILED {key}: {exc} ===", flush=True)
            print(tb, flush=True)

        save_atomic(all_results, OUTPUT_PATH)

        elapsed = time.time() - script_start_time
        avg = elapsed / max(i, 1)
        remaining = len(pending_tasks) - i
        eta_sec = avg * remaining
        eta_min = eta_sec / 60
        print(
            f"  -> saved partial; avg={avg:.0f}s/run; "
            f"ETA={eta_min:.0f}min for {remaining} remaining",
            flush=True,
        )

    summary = build_summary(all_results)
    save_atomic(summary, SUMMARY_PATH)
    print_summary(summary)
    print(f"\nWrote {SUMMARY_PATH}", flush=True)


if __name__ == "__main__":
    main()
