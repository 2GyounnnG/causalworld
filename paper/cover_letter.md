...
We submit our manuscript, “A Model-Conditioned Attribution Protocol for Graph Priors in Node-Wise Scientific Latent Dynamics,” for consideration in the Journal of Computational Science.
...
We propose a model-conditioned attribution protocol that decomposes graph-prior efficacy into three operational attribution classes: controlled audit-positive / topology-aligned support, where graph-favored rollout point estimates agree with separated latent audit metrics; generic regularization, where the graph prior improves over no-prior training but does not survive SMPG controls; and no effect.
...
The contribution is methodological rather than architectural: we provide a practical model-conditioned attribution workflow for prior selection and interpretation under a fixed model condition. We do not introduce a new graph prior, claim universal physics prediction, frame the task as causal discovery, or present a state-of-the-art benchmark.

We believe the manuscript is a good fit for the Journal of Computational Science because it addresses a recurring methodological hazard in computational physics, molecular machine learning, and graph-based scientific modeling. The protocol is implemented as an adapter-based attribution tool with standardized run artifacts, and the code is available at:
https://github.com/2GyounnnG/causalworld

The manuscript has not been published elsewhere and is not under consideration by any other journal. The author declares no conflicts of interest. Generative AI tools were not used to generate scientific results, numerical analyses, datasets, raw experimental artifacts, or submitted figures. All submitted figures are manually redrawn or produced using deterministic plotting code.
