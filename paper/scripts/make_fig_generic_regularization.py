...
effect_vs_none = [6.014, 4.768, 4.824, 8.367, 5.196, 3.979, 6.785, 2.359]
effect_vs_smpg = [0.535, -0.370, 0.906, 0.147, -2.935, -0.445, 0.191, -0.289]
...
ax.bar(x - width/2, effect_vs_none, width, yerr=ci_none, capsize=4,
       label='vs no-prior baseline', color='#2c7bb6', alpha=0.85)
ax.bar(x + width/2, effect_vs_smpg, width, yerr=ci_smpg, capsize=4,
       label='vs SMPG control', color='#d7191c', alpha=0.85)
ax.axhline(y=0, color='black', linestyle='-', linewidth=0.5)
ax.axhline(y=2, color='gray', linestyle='--', linewidth=0.5,
           label='|effect|=2 reference')
ax.axhline(y=-2, color='gray', linestyle='--', linewidth=0.5)
ax.set_ylabel('Effect size (z-score)', fontsize=11)
